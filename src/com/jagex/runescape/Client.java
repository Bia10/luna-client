...
import com.jagex.runescape.definition.IdentityKit;
...
public final class Client extends RSApplet {
    ...
    public URL getCodeBase() {
        ...
        try {
            if (super.gameFrame != null) {
                ...
                return new URL(Constants.HOST + (80 + portOffset));
            }
        } catch (final Exception _ex) {
        }
    ...
    private void login(final String playerUsername, final String playerPassword, final boolean recoveredConnection) {
        ...
        try {
            ...
            this.socket = new RSSocket(this, this.openSocket(Constants.PORT + portOffset));
    ...
    public Socket openSocket(final int port) throws IOException {
        if (signlink.applet != null) {
            ...
            return new Socket(InetAddress.getByName(Constants.HOST), port);
        }
    }
    ...
    void startUp() {
        ...
        try {
            ...
            try {
                for (int i = 0; i < 3; i++) {
                    this.skullIcons[i] = new Sprite(archiveMedia, "headicons_pk", i);
                }
            } catch (final Exception _ex) {
                _ex.printStackTrace();
            }
            try {
                for (int i = 0; i < 8; i++) {
                    this.headIcons[i] = new Sprite(archiveMedia, "headicons_prayer", i);
                }
            } catch (final Exception _ex) {
                _ex.printStackTrace();
            }
            try {
                for (int i = 0; i < 2; i++) {
                    this.hintIcons[i] = new Sprite(archiveMedia, "headicons_hint", i);
                }
            } catch (final Exception _ex) {
                _ex.printStackTrace();
            }
    ...
    private void updateNPCList(final int amount, final Buffer stream) {
        while (stream.bitPosition + 21 < amount * 8) {
            final int npcId = stream.readBits(Constants.NPC_BITS);
            if (npcId == 16383) {
                break;
            }
